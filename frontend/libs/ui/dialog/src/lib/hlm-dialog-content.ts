...
@Component({
  ...
  template: `
		...
		@if (showCloseButton()) {
			<button hlmBtn variant="ghost" size="icon-sm" class="absolute inset-e-2 top-2" hlmDialogClose>
...
export class HlmDialogContent
{
  ...
  public readonly showCloseButton = input<boolean, BooleanInput>(this._dialogContext?.$showCloseButton ?? true, { transform: booleanAttribute })
