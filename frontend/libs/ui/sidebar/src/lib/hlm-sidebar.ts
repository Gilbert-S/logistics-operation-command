  ...
  template: `
		...
		@if (collapsible() === 'none') {
			...
			>
				<hlm-sheet-content
					*hlmSheetPortal="let ctx"
					...
					data-mobile="true"
					class="h-svh w-(--sidebar-width) bg-sidebar p-0 text-sidebar-foreground [&>button]:hidden"
			...
			<div data-slot="sidebar-container" [attr.data-side]="_dataSide()" [class]="_sidebarContainerComputedClass()">
				<div data-sidebar="sidebar" data-slot="sidebar-inner" class="
...
export class HlmSidebar
{
  ...
  protected readonly _dataSlot = computed(() =>
    !this._sidebarService.isMobile() ? "sidebar" : undefined)
